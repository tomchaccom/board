...
// 수정 처리 -- upload.single('attachment') 추가
router.post('/edit/:id', upload.single('attachment'),(req, res) => {
    const { title, content } = req.body;
    console.log('Received title:', title); // <-- 이 줄 추가
    console.log('Received content:', content); // <-- 이 줄 추가
    ...
    db.run(
        ...
        (err) => {
            if (err){
                console.error('게시글 수정 DB 오류:', err.message);
